...
int pic_decode(const char *filePath, int *width, int *height, unsigned char **rgbaBuf) {
    ...
    if (pFormatCtx == NULL) {
        LOGE(LOG_TAG, "Alloc avformat_context Error");
        return -1;
    }

    if (avformat_open_input(&pFormatCtx, filePath, NULL, NULL) != 0) {
        LOGE(LOG_TAG, "avformat_open_input Error");
        return -1;
    }

    if (avformat_find_stream_info(pFormatCtx, NULL ) < 0) {
        LOGE(LOG_TAG, "avformat_find_stream_info Error");
        return -1;
    }
    ...
    while(!finished) {
        ...
        if (ret < 0) {
            LOGE(LOG_TAG, "av_read_frame return an error");
            if (ret != AVERROR_EOF) {
                LOGE(LOG_TAG, "av_read_frame return an not AVERROR_EOF error");
            } else {
                LOGI(LOG_TAG, "video EOF");
            }
        ...
        if (packet.stream_index == videoStreamIndex) {
            ...
            if (len < 0) {
                LOGI(LOG_TAG, "decode video error, skip packet");
            }
            if (got_picture) {
                finished = true;
            }
    ...
    if (AV_PIX_FMT_RGBA != videoFrame->format) {
        LOGI(LOG_TAG, "Pic Frame Pix Format is not RGBA(%d), Format: %d", AV_PIX_FMT_RGBA, videoFrame->format );
        LOGI(LOG_TAG, "need convert pix format");
        SwsContext *pSwsCtx;
        ...
        if (NULL == pSwsCtx) {
            LOGE(LOG_TAG, "sws_getContext Error");
        }
        AVPicture picture;
        ...
        LOGI(LOG_TAG, "picture linesize : %d, frame linesize : %d", picture.linesize[0], videoFrame->linesize[0]);
